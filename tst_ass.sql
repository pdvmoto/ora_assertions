...
pre-requirement: make sure to have EMP/DEPT from demobld  
...
examples of audit_definitions:
 ...
 - todo: infractions: count infr per emp, applies to emp
 - todo: crimrec: applies to EMP only, not to dept or total
 - todo: nr_reports_to : sum of all emps under 1 manager
 - more..
...
notes, etc..
 ...
 - future version: use A_DEF to spool and then run the inserts that generate A_FND-ings
...
-- constrain to parent audit item
alter table a_fnd add constraint  a_fnd_fk_def
                     foreign key  (a_def_id) references a_def ( id) ;

-- two constraints, fks to dept and emp, the arc
alter table a_fnd add constraint  a_fnd_fk_dept
                     foreign key  (deptno) references dept (deptno) ;

alter table a_fnd add constraint  a_fnd_fk_emp
                     foreign key  (empno)  references  emp (empno) ;

-- X-arc: check that only one of dept/emp FKs can be used, other must be NULL
-- also allowed: both-null, signify the defnition applies to Total, or "other" 
...
/* 
the additional contraint: an Assertion.
  an A_DEF defined for level DEPT, is not allowed to have an FNDing at EMP level
  and vice-versa
*/
...
-- for the record: try an invalid type
insert into a_def ( a_name, a_lvl, a_source, a_desc, a_sql  )
values ( 'invalid?'
...
-- note: we need valid empno, if we try insert d.deptno, we get the FK-error, 
-- hence, an FK fires before Assertion (and that makes sense)
--
insert into a_fnd ( a_def_id, empno, n_result ) 
select ad.id
     , max(e.empno)     -- make sure we pick a valid Empno...
     , count (*) 
...
-- try to store emp-level data with a deptno
-- 
insert into a_fnd ( a_def_id, deptno, n_result ) 
select ad.id
     , d.deptno 
     , avg ( sal + nvl ( comm , 0 ) ) -- need some grouping
